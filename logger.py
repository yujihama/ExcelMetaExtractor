...
class Logger:
    ...
    def method_start(self, method_name):
        """メソッドの開始をログに記録"""
        self.logger.info(
            f"============== Method Start: {method_name} ==============")

    def method_end(self, method_name):
        """メソッドの終了をログに記録"""
        self.logger.info(
            f"============== Method End: {method_name} ==============")
    ...
    def region_detected(self, region_type, range_str):
        """領域検出をログに記録"""
        self.logger.info(
            f"[Region Detected] Type: {region_type}, Range: {range_str}")

    def processing_region(self, region_type, range_str):
        """領域処理をログに記録"""
        self.logger.info(
            f"[Processing Region] Type: {region_type}, Range: {range_str}")
    ...
    def start_region_processing(self, region_info):
        ...
        self.logger.info(
            f">>>>> Start Processing Region - Type: {region_type}, Range: {range_str}"
        )
    ...
    def end_region_processing(self, region_info):
        ...
        range_str = region_info.get("range", "N/A")
        self.logger.info(
            f"<<<<< End Processing Region - Type: {region_type}, Range: {range_str}"
        )
    ...
    def debug_region(self, row, col, value, region_type=None):
        """領域のデバッグ情報をログに記録"""
        self.logger.info(
            f"Processing cell at row={row}, col={col}, value={value}, detected_type={region_type}"
        )

    def debug_boundaries(self, start_row, start_col, max_row, max_col):
        """領域境界のデバッグ情報をログに記録"""
        self.logger.info(
            f"Region boundaries: ({start_row},{start_col}) to ({max_row},{max_col})"
        )
